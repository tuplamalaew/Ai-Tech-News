...
export async function GET(request: Request) {
  try {
    ...
    // 1. Fetch raw data from Hacker News & Dev.to
    console.log('1. Fetching raw news from sources...');
    const rawArticles = await fetchAllTechNews(10); // Fetch top 10 from each

    if (rawArticles.length === 0) {
      return NextResponse.json({ success: false, message: 'No articles fetched.' });
    }

    // 2. Process with LLM (Google Gemini)
    console.log(`2. Processing ${rawArticles.length} articles with LLM...`);
    const processedArticles = await filterAndSummarizeNews(rawArticles);
